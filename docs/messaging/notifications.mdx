...
FCM based notificiations provide the basics for many usecases, such as displaying text and images. They do not support advanced notifications
...
If your application is in the foreground & you wish for incoming messages containing a notification property to display a notification to users
(called a "heads up" notification), some additional configuration is required. This configuration is different for Android & iOS.

#### iOS Configuration

For iOS, we need to alter the presentation options for incoming notifications. To do this, call the `setForegroundNotificationPresentationOptions` method:
...
schema within the `application` block:
...
Rebuild your application (`flutter run`) and incoming notification messages will now use your channel. Due to the "high"
importance, this allows notifications to be dislayed in a heads up mode.
