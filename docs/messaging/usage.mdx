...
Messaging currently only supports usage with the default Firebase App.
...
### Requesting permission (iOS, Web)

On iOS & web, before FCM payloads can be recieved on your device, you must first ask the users permission. Android applications are not required
...
the method requests sensible default permissions. The reference API provides full documnentation on what each permission is for.
...
A single message payload can be viewed as one of three types:
...
3. Notification & Data messages: Payloads with both a `notification` and `data` properties.
...
2. It must be a top-level function (e.g. not a class method which requires initialisation).
...
If your message is a notification one (includes a `notification` property), the Firebase SDKs will intercept this and display a visble
...
Due to the differences in how Android & iOS handle background messages, debugging and hot reload works in Android only.

On iOS, confirming the background handler has executed can be tricky. In general the "console.app" application on OSX displays
system logs, including those from Flutter. There is currently no way to debug within the background handler on iOS.

#### Low priority messages

As mentioned above, data only messages are classed as "low priority". Devices can (and probably will) ignore these messages if your
application is in the background or terminated. You should not rely data only messages to be delivered, they should only be used to
support your applications functionaly in non-critical functionality.

To help improve delivery, you can bump the priority of messages. Note; this does still not guarentee delivery.
...
admin.initializeApp({
  credential: admin.credential.cert(requre("./service-account-file.json")),
...
descriptions on the use of the properties on the [official documentation](https://firebase.google.com/docs/reference/admin/node/TokenMessage).
...
will cause the device to block messages, or if battery consumption is high they will also be blocked.
...
"console.app" application will display "CANCELED" logs for those it chose to ignore, however doesn't provide much description as to why.
